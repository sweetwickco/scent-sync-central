...
Please generate a comprehensive action plan with specific, actionable tasks. Each task should have:
1. A clear, concise title
2. A detailed description explaining what needs to be done

Focus on practical, executable steps that will help achieve the stated goal within the given timeline and budget. Consider market research, product development, marketing, operations, financial planning, and any other relevant areas.

Return the response as a JSON object with a "tasks" array containing objects with "title" and "description" properties. Aim for 8-15 tasks total.`;
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'You are a business planning expert. Always respond with valid JSON containing a "tasks" array with actionable business tasks.' 
          },
    ...
    try {
      ...
      // Create a fallback response with some default tasks
      parsedTasks = {
        tasks: [
          {
